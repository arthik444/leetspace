...
# Select verification strategy based on environment
if os.getenv("USE_DEV_AUTH", "0") == "1":
...
async def get_current_user(credentials: HTTPAuthorizationCredentials = Depends(security)) -> dict:
	...
	# Check if email is verified (configurable; default required in production)
	require_email_verified = os.getenv("REQUIRE_EMAIL_VERIFIED", "1") == "1"
	if require_email_verified and not user_info.get("email_verified", False):
		raise HTTPException(
			status_code=status.HTTP_401_UNAUTHORIZED,
